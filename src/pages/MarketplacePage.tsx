...
import { addUserOffsetRetirement } from '../lib/userDataService';
...
export const MarketplacePage: React.FC<MarketplacePageProps> = ({ onNavigate, suggestedOffsetTonnes = 24.6 }) => {
  ...
  const [beneficiaryName, setBeneficiaryName] = useState(userProfile?.organization || userProfile?.displayName || 'Enterprise Climate Account');
  ...
  const handleSimulateRetirement = async () => {
    const serial = `CSTK-RET-${Math.floor(100000 + Math.random() * 900000)}-${selectedProject.standard.replace(/[^a-zA-Z0-9]/g, '')}`;
    ...
    if (user) {
      try {
        await addUserOffsetRetirement(user.uid, {
          ...
          registry: selectedProject.standard,
          serialNumber: serial,
          tonnes: quantityTonnes,
          costUsd: Number(totalCost),
        });
      } catch (err) {
        console.warn('Could not auto-save retirement to firestore:', err);
      }
    }
  };
          ...
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 sm:p-8 space-y-6 shadow-2xl border border-slate-200 relative">
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-xs font-mono text-slate-400">
                SERIAL: {generatedSerial || `CSTK-RET-994182-${selectedProject.standard.replace(/[^a-zA-Z0-9]/g, '')}`}
              </p>
            </div>
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2">
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-900 font-semibold text-xs hover:bg-emerald-100"
              >
              ...
              <button
                ...
                className="px-5 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs flex items-center gap-1.5"
              >
